...
def parse_template(inherits, source, delimiter):
    ...
    with open(source, "r", encoding="utf-8") as f:
        ...
        if script_template["space-indent"] != "":
            ...
            script_template["script"] = script_template["script"].replace(
                indent, "_TS_")
        if script_template["name"] == "":
            script_template["name"] = os.path.splitext(os.path.basename(source))[
                0].replace("_", " ").title()
        script_template["script"] = (
            script_template["script"].replace(
                '"',
                '\\"').lstrip().replace(
                "\n",
                "\\n").replace(
...
def make_templates(target, source, env):
    ...
    with StringIO() as s:
        ...
        s.write(
            "\nstatic const int TEMPLATES_ARRAY_SIZE = " +
            str(number_of_templates) +
            ";\n")
        s.write(
            "\nstatic const struct ScriptLanguage::ScriptTemplate TEMPLATES[" +
            str(number_of_templates) +
            "] = {\n")
