...
uint32_t CompilerMSL::add_interface_block(StorageClass storage, bool patch) {
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable &var) {
		...
		if (get_execution_model() == ExecutionModelFragment && storage == StorageClassOutput && !patch &&
				((is_builtin && ((bi_type == BuiltInFragDepth && (!msl_options.enable_frag_depth_builtin || uses_explicit_early_fragment_test())) || (bi_type == BuiltInFragStencilRefEXT && (!msl_options.enable_frag_stencil_ref_builtin || uses_explicit_early_fragment_test())))) ||
...
uint32_t CompilerMSL::add_interface_block_pointer(uint32_t ib_var_id, StorageClass storage) {
	...
	if (is_tesc_shader() || (is_tese_shader() && msl_options.raw_buffer_tese_input)) {
		...
		ib_ptr_type.storage = storage == StorageClassInput ? ((is_tesc_shader() && msl_options.multi_patch_workgroup) ||
																					 (is_tese_shader() && msl_options.raw_buffer_tese_input)
...
#define MSL_AFMIO(op) MSL_AFMO_IMPL(op, 1, true)
		...
		case OpImageTexelPointer: {
			...
			if (var && atomic_image_vars_emulated.count(var->self)) {
				...
				auto &e =
						set<SPIRExpression>(id, join(to_expression(ops[2]), "@", bitcast_expression(SPIRType::UInt, ops[3])),
								result_type, true);
...
#define MSL_ImgQry(qrytype)                                                                 \
	do {                                                                                    \
		...
		case OpIsHelperInvocationEXT:
			...
			emit_op(ops[0], ops[1],
					needs_manual_helper_invocation_updates() ? builtin_to_glsl(BuiltInHelperInvocation, StorageClassInput) : "simd_is_helper_thread()",
					false);
			break;
...
#undef MSL_RAY_QUERY_OP_INNER2
		...
		case OpSUDotAccSat: {
			...
			if (instruction.length == 6) {
				...
				// Inputs are sign or zero-extended to their target width.
				SPIRType::BaseType vec1_expected_type =
						opcode != OpUDotAccSat ? to_signed_basetype(input_type1.width) : to_unsigned_basetype(input_type1.width);

				SPIRType::BaseType vec2_expected_type =
						opcode != OpSDotAccSat ? to_unsigned_basetype(input_type2.width) : to_signed_basetype(input_type2.width);
