	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable &var) {
		...
		if (is_builtin) {
			...
			if (has_decoration(type.self, DecorationBlock)) {
				...
				bool strip_array =
						!has_decoration(var.self, DecorationPatch) && (get_execution_model() == ExecutionModelTessellationControl || (get_execution_model() == ExecutionModelTessellationEvaluation && var.storage == StorageClassInput));
