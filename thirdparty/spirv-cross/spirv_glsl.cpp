...
{
	subpass_to_framebuffer_fetch_attachment.emplace_back( input_attachment_index, color_location );
	inout_color_attachments.emplace_back( color_location, coherent );
}
...
string CompilerGLSL::layout_for_variable(const SPIRVariable &var)
{
	...
	auto &type = get<SPIRType>(var.basetype);
	const auto &flags = get_decoration_bitset(var.self);
	const auto &typeflags = get_decoration_bitset(type.self);
...
void CompilerGLSL::emit_buffer_reference_block(uint32_t type_id, bool forward_declaration)
{
	...
	if (!forward_declaration)
	{
		...
		{
			const auto &pointee_type = get_pointee_type(type);
...
{
	const auto &entry_point = get_entry_point();
	int index = -1;
...
void CompilerGLSL::fixup_image_load_store_access()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t var, const SPIRVariable &) {
		const auto &vartype = expression_type(var);
...
bool CompilerGLSL::variable_is_lut(const SPIRVariable &var) const
{
	...
	{
		const auto *constant = maybe_get<SPIRConstant>(var.static_expression);
...
#undef GLSL_UOP
	...
	else if (cop.opcode == OpSConvert || cop.opcode == OpUConvert)
	{
		...
		const auto &arg_type = expression_type(cop.arguments[0]);
                                         ...
                                         bool inside_struct_scope)
{
	...
	else if (!c.subconstants.empty())
	{
		...
		for (const auto &elem : c.subconstants)
		{
	...
	else if (type.basetype == SPIRType::Struct && type.member_types.empty())
	{
...
void CompilerGLSL::emit_uninitialized_temporary(uint32_t result_type, uint32_t result_id)
{
	...
	else if (hoisted_temporaries.count(result_id) == 0)
	{
		...
		const auto &flags = get_decoration_bitset(result_id);
...
{
	const auto *expr = maybe_get<SPIRExpression>(id);
	if (!expr) {
		return false;
}
...
{
	const auto &type0 = expression_type(op0);
	const auto &type1 = expression_type(op1);
...
bool CompilerGLSL::emit_complex_bitcast(uint32_t result_type, uint32_t id, uint32_t op0)
{
	...
	const auto &input_type = expression_type(op0);
	string expr;
...
{
	auto &out_type = get<SPIRType>(result_type);
	const auto &expr_type = expression_type(op0);
...
bool CompilerGLSL::to_trivial_mix_op(const SPIRType &type, string &op, uint32_t left, uint32_t right, uint32_t lerp)
{
	...
	const auto &lerptype = expression_type(lerp);
...
{
	string expr;
	const auto &lerptype = expression_type(select);
...
bool CompilerGLSL::is_supported_subgroup_op_in_opengl(spv::Op op, const uint32_t *ops)
{
	...
	case OpGroupNonUniformFMul:
	{
		...
		return operation == GroupOperationReduce || operation == GroupOperationInclusiveScan ||
                                        ...
                                        SmallVector<uint32_t> &inherited_expressions)
{
	...
	const auto &type = expression_type(img);
...
{
	const auto *c = maybe_get<SPIRConstant>(id);
	if (!c) {
		return false;
}
...
{
	const auto &type = expression_type(ptr);
...
string CompilerGLSL::to_function_name(const TextureFunctionNameArguments &args)
{
	...
	string fname;
	const auto &imgtype = *args.base.imgtype;
...
string CompilerGLSL::to_function_args(const TextureFunctionArguments &args, bool *p_forward)
{
	...
	const auto *swizzle_expr = swizzle(args.coord_components, expression_type(args.coord).vecsize);
	...
	const auto &coord_type = expression_type(args.coord);
...
void CompilerGLSL::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t *args, uint32_t length)
{
	...
	case GLSLstd450Round:
		...
		} else
		{
			...
			const auto &op0_type = expression_type(args[0]);
	...
	case GLSLstd450Modf:
		...
		else
		{
			...
			const auto &op1_type = expression_type(args[1]);
	...
	case GLSLstd450Determinant:
	{
		...
		{
			const auto &type = expression_type(args[0]);
...
std::string CompilerGLSL::bitcast_expression(SPIRType::BaseType target_type, uint32_t arg)
{
	...
	const auto &src_type = expression_type(arg);
                                           ...
                                           AccessChainFlags flags, AccessChainMeta *meta)
{
	...
	for (uint32_t i = 0; i < count; i++)
	{
		...
		if (ptr_chain && i == 0)
		{
			...
			{
				dimension_flatten = !type->array.empty();
	...
	else
	{
		...
		if (ptr_chain)
		{
			...
			if (backend.native_pointers && has_decoration(type_id, DecorationArrayStride))
			{
				...
				const auto &pointee_type = get_pointee_type(get<SPIRType>(type_id));
...
{
	const auto &type = expression_type(lhs_id);
...
bool CompilerGLSL::should_dereference(uint32_t id)
{
	...
	if (auto *expr = maybe_get<SPIRExpression>(id))
	{
		...
		{
			const auto &src_type = expression_type(expr->loaded_from);
...
bool CompilerGLSL::should_forward(uint32_t id) const
{
	...
	const auto *var = maybe_get<SPIRVariable>(id);
	if (var)
	{
...
void CompilerGLSL::disallow_forwarding_in_expression_chain(const SPIRExpression &expr)
{
	    ...
	    forced_invariant_temporaries.count(expr.self) == 0)
	{
		...
		for (const auto &dependent : expr.expression_dependencies) {
...
#define GLSL_UFOP(op) emit_unary_func_op(ops[0], ops[1], ops[2], #op)
	...
	case OpLoad:
	{
		...
		auto &type = get<SPIRType>(result_type);
		const auto &expr_type = expression_type(ptr);
	...
	case OpCompositeConstruct:
	{
		...
		auto &out_type = get<SPIRType>(result_type);
		const auto *in_type = length > 0 ? &expression_type(elems[0]) : nullptr;
	...
	case OpCompositeExtract:
	{
		...
		const auto &composite_type = expression_type(ops[2]);
	...
	case OpVectorShuffle:
	{
		...
		const auto &type0 = expression_type(vec0);
	...
	case OpFRem:
	{
		...
		else
		{
			...
			const auto &op0_type = expression_type(op0);
			auto via_type = op0_type;
			via_type.basetype = SPIRType::Int;
	...
	case OpConvertUToF:
	{
		...
		auto &type = get<SPIRType>(result_type);
		const auto &arg_type = expression_type(ops[2]);
	...
	case OpConvertFToS:
	{
		...
		const auto &float_type = expression_type(ops[2]);
	...
	case OpAtomicLoad:
	{
		...
		const auto &type = expression_type(ops[2]);
	...
	case OpImageWrite:
	{
		...
		const auto &value_type = expression_type(ops[2]);
	...
	case OpMemoryBarrier:
	{
		...
		if (opcode == OpMemoryBarrier)
		{
			...
			{
				const auto *next_ops = stream(*next);
...
#undef GLSL_RAY_QUERY_GET_OP2
	...
	case OpConvertUToPtr:
	{
		...
		const auto &in_type = expression_type(ops[2]);
	...
	{
		auto &type = get<SPIRType>(ops[0]);
		const auto &ptr_type = expression_type(ops[2]);
...
{
	const auto &flags = get_decoration_bitset(id);
	string res;
...
string CompilerGLSL::argument_decl(const SPIRFunction::Parameter &arg)
{
	...
	const auto &type = expression_type(arg.id);
...
bool CompilerGLSL::type_can_zero_initialize(const SPIRType &type) const
{
	...
	for (const auto &literal : type.array_size_literal) {
		if (!literal) {
			return false;
}
}
...
{
	const auto &flags = get_decoration_bitset(variable.self);
	if (flags.get(DecorationRelaxedPrecision)) {
		return "mediump ";
	} else {
		return "highp ";
...
string CompilerGLSL::to_array_size(const SPIRType &type, uint32_t index)
{
	...
	const auto &size = type.array[index];
	if (!type.array_size_literal[index]) {
...
string CompilerGLSL::type_to_glsl(const SPIRType &type, uint32_t id)
{
	if (is_physical_pointer(type) && !is_physical_pointer_to_buffer_block(type))
	{
		...
		const auto *parent = &get_pointee_type(type);
...
{
	const auto *itr = find(begin(forced_extensions), end(forced_extensions), ext);
...
void CompilerGLSL::flatten_buffer_block(VariableID id)
{
	...
	const auto &flags = get_decoration_bitset(type.self);
...
void CompilerGLSL::branch_to_continue(BlockID from, BlockID to)
{
	...
	else
	{
		...
		if (loop_dominator != 0)
		{
			const auto &cfg = get_cfg_for_current_function();
...
void CompilerGLSL::emit_while_loop_initializers(const SPIRBlock &block)
{
	...
	for (const auto &loop_var : block.loop_variables)
	{
...
string CompilerGLSL::emit_for_loop_initializers(const SPIRBlock &block)
{
	...
	for (const auto &variable : block.loop_variables)
	{
...
bool CompilerGLSL::for_loop_initializers_are_same_type(const SPIRBlock &block)
{
	...
	for (const auto &var : block.loop_variables)
	{
...
void CompilerGLSL::emit_hoisted_temporaries(SmallVector<pair<TypeID, ID>> &temporaries)
{
	...
	for (auto &tmp : temporaries)
	{
		...
		const auto &flags = get_decoration_bitset(tmp.second);
		...
		if (mirrored_precision_itr != temporary_to_mirror_precision_alias.end())
		{
			...
			const auto &mirror_flags = get_decoration_bitset(mirror_id);
...
void CompilerGLSL::emit_block_chain(SPIRBlock &block)
{
	...
	{
		const auto &type = expression_type(block.condition);
		...
		const auto &cases = get_case_list(block);
	...
	case SPIRBlock::Return:
	{
		...
		{
			const auto &type = expression_type(block.return_value);
...
bool CompilerGLSL::unroll_array_to_complex_store(uint32_t target_id, uint32_t source_id)
{
	...
	const auto &type = expression_type(source_id);
	string array_expr;
...
bool CompilerGLSL::subpass_input_is_framebuffer_fetch(uint32_t id) const
{
	...
	for (const auto &remap : subpass_to_framebuffer_fetch_attachment) {
...
void CompilerGLSL::emit_inout_fragment_outputs_copy_to_subpass_inputs()
{
	...
	{
		const auto *subpass_var = find_subpass_input_by_attachment_index(remap.first);
		const auto *output_var = find_color_output_by_location(remap.second);
		if (!subpass_var) {
			continue;
}
		if (!output_var) {
...
{
	const auto &block_type = get<SPIRType>(var.basetype);
...
{
	const auto &type = strip_array ? get_variable_element_type(var) : get_variable_data_type(var);
	uint32_t location = get_decoration(var.self, DecorationLocation);

	for (uint32_t i = 0; i < mbr_idx; i++)
	{
		const auto &mbr_type = get<SPIRType>(type.member_types[i]);
