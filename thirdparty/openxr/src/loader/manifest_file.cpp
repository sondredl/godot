...
static void ReadRuntimeDataFilesInRegistry(const std::string &runtime_registry_location,
	...
	if (ERROR_SUCCESS != RegGetValueW(hkey, nullptr, default_runtime_value_name_w.c_str(), RRF_RT_REG_SZ | REG_EXPAND_SZ | RRF_ZEROONFAILURE, NULL, reinterpret_cast<LPBYTE>(&value_w), &value_size_w)) {
		LoaderLogger::LogWarningMessage(
