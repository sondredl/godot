...
bool ShiftCollider::mergeSlot(Segment *seg, Slot *slot, const SlotCollision *cslot, const Position &currShift,
	...
	if (sameCluster && _seqClass && (sameClass || (_seqProxClass != 0 && cslot->seqClass() == _seqProxClass)))
	...
	if (orderFlags || (sx + bb.xa + _margin >= _limit.bl.x && sx + bb.xi - _margin <= _limit.tr.x) || (sy + bb.ya + _margin >= _limit.bl.y && sy + bb.yi - _margin <= _limit.tr.y))
...
#define DBGTAG(x) \
	if (dbgout)   \
		dbgout->setenv(1, reinterpret_cast<void *>(-x));
#else
#define DBGTAG(x)
#endif
			...
			if (orderFlags) {
				...
				switch (orderFlags) {
					...
					case SlotCollision::SEQ_ORDER_LEFTDOWN: {
						...
						// region 5
						DBGTAG(25)
						addBox_slope(false, Rect(Position(xminf, r2Yedge - cslot->seqValignHt()), Position(sx + bb.xa, r2Yedge)), tbb, tsb, org, seq_below_wt, seq_valign_wt, false, i);
						break;
					}
					...
					case SlotCollision::SEQ_ORDER_NOLEFT: // enforce neighboring glyph being to the left
						...
						removeBox(Rect(Position(xminf, bb.yi - tbb.ya + sy),
										  Position(bb.xi - tbb.xa + sx, bb.ya - tbb.yi + sy)),
								tbb, tsb, org, i);
						break;
					case SlotCollision::SEQ_ORDER_NORIGHT: // enforce neighboring glyph being to the right
						...
						removeBox(Rect(Position(bb.xa - tbb.xi + sx, bb.yi - tbb.ya + sy),
										  Position(xpinf, bb.ya - tbb.yi + sy)),
								tbb, tsb, org, i);
						break;
					default:
						break;
...
Position ShiftCollider::resolve(GR_MAYBE_UNUSED Segment *seg, bool &isCol, GR_MAYBE_UNUSED json *const dbgout) {
	...
	for (int i = 0; i < 4; ++i) {
		...
		if (bestCost >= 0.0f) {
			...
			switch (i) {
				case 0:
					testp = Position(bestPos, _currShift.y);
					break;
				case 1:
					testp = Position(_currShift.x, bestPos);
					break;
				case 2:
					testp = Position(0.5f * (_currShift.x - _currShift.y + bestPos), 0.5f * (_currShift.y - _currShift.x + bestPos));
					break;
				case 3:
					testp = Position(0.5f * (_currShift.x + _currShift.y + bestPos), 0.5f * (_currShift.x + _currShift.y - bestPos));
					break;
			}
...
inline static float localmax(float al, float au, float bl, float bu, float x) {
	if (al < bl) {
		if (au < bu)
			return au < x ? au : x;
	} else if (au > bu)
...
inline static float localmin(float al, float au, float bl, float bu, float x) {
	if (bl > al) {
		if (bu > au)
			return bl > x ? bl : x;
	} else if (au > bu)
