...
public:
	...
	_glat_iterator(const void *glat = 0) :
			_e(reinterpret_cast<const byte *>(glat)), _v(_e + 2 * sizeof(W)), _n(0) {}
...
GlyphCache::GlyphCache(const Face &face, const uint32 face_options) :
		...
		_glyphs(_glyph_loader && *_glyph_loader && _glyph_loader->num_glyphs()
						? grzeroalloc<const GlyphFace *>(_glyph_loader->num_glyphs())
						: 0),
		_boxes(_glyph_loader && _glyph_loader->has_boxes() && _glyph_loader->num_glyphs()
						? grzeroalloc<GlyphBox *>(_glyph_loader->num_glyphs())
						: 0),
...
const GlyphFace *GlyphCache::glyph(unsigned short glyphid) const //result may be changed by subsequent call with a different glyphid
{
	...
	if (p == 0 && _glyph_loader) {
		...
		if (g)
			p = _glyph_loader->read_glyph(glyphid, *g, &numsubs);
		if (!p) {
			delete g;
			return *_glyphs;
		}
...
GlyphCache::Loader::Loader(const Face &face) :
	...
	if ((m_pGlat = Face::Table(face, Tag::Glat, 0x00030000)) == NULL || (m_pGloc = Face::Table(face, Tag::Gloc)) == NULL || m_pGloc.size() < 8) {
		_head = Face::Table();
		return;
	}
	...
	_long_fmt = flags & 1;
	ptrdiff_t tmpnumgattrs = (m_pGloc.size() - (p - m_pGloc) - sizeof(uint16) * (flags & 0x2 ? _num_attrs : 0)) / (_long_fmt ? sizeof(uint32) : sizeof(uint16)) - 1;

	if (version >= 0x00020000 || tmpnumgattrs < 0 || tmpnumgattrs > 65535 || _num_attrs == 0 || _num_attrs > 0x3000 // is this hard limit appropriate?
			|| _num_glyphs_graphics > tmpnumgattrs || m_pGlat.size() < 4) {
		_head = Face::Table();
		return;
	}
...
inline GlyphCache::Loader::operator bool() const throw() {
	return _head && _hhea && _hmtx && !(bool(_glyf) != bool(_loca));
}

inline unsigned short int GlyphCache::Loader::units_per_em() const throw() {
	return _head ? TtfUtil::DesignUnits(_head) : 0;
}

inline unsigned short int GlyphCache::Loader::num_glyphs() const throw() {
	return max(_num_glyphs_graphics, _num_glyphs_attributes);
}

inline unsigned short int GlyphCache::Loader::num_attrs() const throw() {
	return _num_attrs;
}

inline bool GlyphCache::Loader::has_boxes() const throw() {
	return _has_boxes;
}
...
const GlyphFace *GlyphCache::Loader::read_glyph(unsigned short glyphid, GlyphFace &glyph, int *numsubs) const throw() {
	...
	if (glyphid < _num_glyphs_attributes) {
		...
		if (glat_version < 0x00020000) {
			if (gloce - glocs < 2 * sizeof(byte) + sizeof(uint16) || gloce - glocs > _num_attrs * (2 * sizeof(byte) + sizeof(uint16)))
				return 0;
			...
			if (gloce - glocs < 3 * sizeof(uint16) // can a glyph have no attributes? why not?
					|| gloce - glocs > _num_attrs * 3 * sizeof(uint16) || glocs > m_pGlat.size() - 2 * sizeof(uint16))
				return 0;
