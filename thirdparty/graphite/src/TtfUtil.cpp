...
bool CheckHeader(const void *pHdr) {
	const Sfnt::OffsetSubTable *pOffsetTable = reinterpret_cast<const Sfnt::OffsetSubTable *>(pHdr);
...
bool GetTableDirInfo(const void *pHdr, size_t &lOffset, size_t &lSize) {
	...
	lSize = be::swap(pOffsetTable->num_tables) * sizeof(Sfnt::OffsetSubTable::Entry);
...
bool CheckTable(const Tag TableId, const void *pTable, size_t lTableSize) {
	...
	if (pTable == 0 || lTableSize < 4)
		return false;

	switch (TableId) {
		case Tag::cmap: // cmap
		{
			const Sfnt::CharacterCodeMap *const pCmap = reinterpret_cast<const Sfnt::CharacterCodeMap *>(pTable);
		...
		case Tag::head: // head
		{
			const Sfnt::FontHeader *const pHead = reinterpret_cast<const Sfnt::FontHeader *>(pTable);
			if (lTableSize < sizeof(Sfnt::FontHeader))
				return false;
			bool r = be::swap(pHead->version) == OneFix && be::swap(pHead->magic_number) == FontHeader::MagicNumber && be::swap(pHead->glyph_data_format) == FontHeader::GlypDataFormat && (be::swap(pHead->index_to_loc_format) == FontHeader::ShortIndexLocFormat || be::swap(pHead->index_to_loc_format) == FontHeader::LongIndexLocFormat) && sizeof(FontHeader) <= lTableSize;
			return r;
		}

		case Tag::post: // post
		{
			const Sfnt::PostScriptGlyphName *const pPost = reinterpret_cast<const Sfnt::PostScriptGlyphName *>(pTable);
			if (lTableSize < sizeof(Sfnt::PostScriptGlyphName))
				return false;
			const fixed format = be::swap(pPost->format);
			bool r = format == PostScriptGlyphName::Format1 || format == PostScriptGlyphName::Format2 || format == PostScriptGlyphName::Format3 || format == PostScriptGlyphName::Format25;
			return r;
		}
		...
		case Tag::hhea: // hhea
		{
			...
			if (lTableSize < sizeof(Sfnt::HorizontalHeader))
				return false;
			bool r = be::swap(pHhea->version) == OneFix && be::swap(pHhea->metric_data_format) == 0 && sizeof(Sfnt::HorizontalHeader) <= lTableSize;
			return r;
		}
		...
		case Tag::maxp: // maxp
		{
			...
			if (lTableSize < sizeof(Sfnt::MaximumProfile))
				return false;
			bool r = be::swap(pMaxp->version) == OneFix && sizeof(Sfnt::MaximumProfile) <= lTableSize;
			return r;
		}

		case Tag::OS_2: // OS/2
		{
			const Sfnt::Compatibility *pOs2 = reinterpret_cast<const Sfnt::Compatibility *>(pTable);
			if (be::swap(pOs2->version) == 0) { // OS/2 table version 1 size
				...
				if (sizeof(Sfnt::Compatibility0) <= lTableSize)
					return true;
			} else if (be::swap(pOs2->version) == 1) { // OS/2 table version 2 size
													   //          if (sizeof(Sfnt::Compatibility)
													   //                  - sizeof(int16) *2
													   //                  - sizeof(uint16)*3 <= lTableSize)
				if (sizeof(Sfnt::Compatibility1) <= lTableSize)
					return true;
			} else if (be::swap(pOs2->version) == 2) { // OS/2 table version 3 size
				if (sizeof(Sfnt::Compatibility2) <= lTableSize)
					return true;
			} else if (be::swap(pOs2->version) == 3 || be::swap(pOs2->version) == 4) { // OS/2 table version 4 size - version 4 changed the meaning of some fields which we don't use
		...
		case Tag::name: {
			const Sfnt::FontNames *pName = reinterpret_cast<const Sfnt::FontNames *>(pTable);
...
int PostLookup(const void *pPost, size_t lPostSize, const void *pMaxp,
	...
	const Sfnt::PostScriptGlyphName *pTable = reinterpret_cast<const Sfnt::PostScriptGlyphName *>(pPost);
	fixed format = be::swap(pTable->format);

	if (format == PostScriptGlyphName::Format3) { // format 3 - no Postscript glyph info in font
		return -2;
	}
	...
	if (format == PostScriptGlyphName::Format1) { // format 1 - use standard Postscript names
		return iPostName;
	}
	...
	if (format == PostScriptGlyphName::Format25) {
		...
		const PostScriptGlyphName25 *pTable25 = static_cast<const PostScriptGlyphName25 *>(pTable);
		int cnGlyphs = GlyphCount(pMaxp);
		for (gid16 nGlyphId = 0; nGlyphId < cnGlyphs && nGlyphId < kcPostNames;
				nGlyphId++) { // glyph_name_index25 contains bytes so no byte swapping needed
	...
	if (format == PostScriptGlyphName::Format2) { // format 2
		const PostScriptGlyphName2 *pTable2 = static_cast<const PostScriptGlyphName2 *>(pTable);

		int cnGlyphs = be::swap(pTable2->number_of_glyphs);

		if (iPostName != -1) { // did match a standard name, look for first glyph id mapped to that name
		...
		{ // did not match a standard name, search font specific names
			...
			bool fFound = false;
			const char *const endOfTable = reinterpret_cast<const char *>(pTable2) + lPostSize;
			while (pGlyphName < endOfTable && !fFound) { // search Pascal strings for first matching name
				size_t nStringSize = size_t(*pGlyphName);
				if (nStrSizeGoal != nStringSize ||
						strncmp(pGlyphName + 1, pPostName, nStringSize)) { // did not match
					++iInNames;
			...
			for (gid16 nGlyphId = 0; nGlyphId < cnGlyphs; nGlyphId++) { // search for first glyph id that maps to the found string index
...
bool HorMetrics(gid16 nGlyphId, const void *pHmtx, size_t lHmtxSize, const void *pHhea,
	...
	if (nGlyphId < cLongHorMetrics) { // glyph id is acceptable
		if ((nGlyphId + 1) * sizeof(Sfnt::HorizontalMetric) > lHmtxSize)
			return false;
...
bool CheckCmapSubtable4(const void *pCmapSubtable4, const void *pCmapEnd /*, unsigned int maxgid*/) {
	...
	if (table_len < sizeof(*pTable) || be::swap(pTable->format) != 4)
		return false;
...
unsigned int CmapSubtable4NextCodepoint(const void *pCmap31, unsigned int nUnicodeId, int *pRangeKey) {
	...
	const uint16 *pStartCode = &(pTable->end_code[0]) + nRange // length of end code array
			+ 1;											   // reserved word
		...
		const void *pHead) // throw (std::out_of_range)
{
	...
	if (be::swap(pTable->index_to_loc_format) == Sfnt::FontHeader::ShortIndexLocFormat) { // loca entries are two bytes and have been divided by two
	...
	} else if (be::swap(pTable->index_to_loc_format) == Sfnt::FontHeader::LongIndexLocFormat) { // loca entries are four bytes
...
bool GlyfPoints(const void *pSimpleGlyf, int *prgnX, int *prgnY,
	...
	const uint8 *pbGlyph = reinterpret_cast<const uint8 *>(&pGlyph->end_pts_of_contours[cContours]);
	...
	int iFlag = 0;
	while (iFlag < cPts) {
		if (!(*pbGlyph & SimpleGlyph::Repeat)) { // flag isn't repeated
			prgbFlag[iFlag] = (char)*pbGlyph;
			pbGlyph++;
			iFlag++;
		} else { // flag is repeated; count specified by next byte
...
bool GlyfContourCount(gid16 nGlyphId, const void *pGlyf, const void *pLoca,
	...
	int cTmp = 0;
	for (size_t i = 0; i < cCompId; i++) {
		if (IsSpace(static_cast<gid16>(rgnCompId[i]), pLoca, lLocaSize, pHead)) {
			return false;
		}
		pSimpleGlyf = GlyfLookup(static_cast<gid16>(rgnCompId[i]),
				pGlyf, pLoca, lGlyfSize, lLocaSize, pHead);
