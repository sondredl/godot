...
bool Pass::readPass(const byte *const pass_start, size_t pass_length, size_t subtable_base,
	...
	if (e.test(pass_length < 40, E_BADPASSLENGTH))
		return face.error(e);
	...
	if (e.test(!m_numRules && m_numCollRuns == 0, E_BADEMPTYPASS))
		return face.error(e);
	...
	if (e.test(m_numTransition > m_numStates, E_BADNUMTRANS) || e.test(m_numSuccess > m_numStates, E_BADNUMSUCCESS) || e.test(m_numSuccess + m_numTransition < m_numStates, E_BADNUMSTATES) || e.test(m_numRules && numRanges == 0, E_NORANGES) || e.test(m_numColumns > 0x7FFF, E_BADNUMCOLUMNS))
		return face.error(e);

	m_successStart = m_numStates - m_numSuccess;
	// test for beyond end - 1 to account for reading uint16
	if (e.test(p + numRanges * 6 - 2 > pass_end, E_BADPASSLENGTH))
		return face.error(e);
	...
	// More sanity checks
	if (e.test(reinterpret_cast<const byte *>(o_rule_map + m_numSuccess * sizeof(uint16)) > pass_end || p > pass_end, E_BADRULEMAPLEN))
		return face.error(e);
	...
	if (e.test(p + 2 * sizeof(uint8) > pass_end, E_BADPASSLENGTH))
		return face.error(e);
	m_minPreCtxt = be::read<uint8>(p);
	m_maxPreCtxt = be::read<uint8>(p);
	if (e.test(m_minPreCtxt > m_maxPreCtxt, E_BADCTXTLENBOUNDS))
		return face.error(e);
	...
	if (e.test(p + sizeof(uint16) + sizeof(uint8) > pass_end, E_BADCTXTLENS))
		return face.error(e);
	m_colThreshold = be::read<uint8>(p);
	if (m_colThreshold == 0)
		m_colThreshold = 10; // A default
	...
	if (e.test(2u * m_numTransition * m_numColumns >= (unsigned)(pass_end - p), E_BADPASSLENGTH) || e.test(p >= pass_end, E_BADPASSLENGTH))
		return face.error(e);
	be::skip<int16>(p, m_numTransition * m_numColumns);
	be::skip<uint8>(p);
	if (e.test(p != pcCode, E_BADPASSCCODEPTR))
		return face.error(e);
	be::skip<byte>(p, pass_constraint_len);
	if (e.test(p != rcCode, E_BADRULECCODEPTR) || e.test(size_t(rcCode - pcCode) != pass_constraint_len, E_BADCCODELEN))
		return face.error(e);
	be::skip<byte>(p, be::peek<uint16>(o_constraint + m_numRules));
	if (e.test(p != aCode, E_BADACTIONCODEPTR))
		return face.error(e);
	be::skip<byte>(p, be::peek<uint16>(o_actions + m_numRules));

	// We should be at the end or within the pass
	if (e.test(p > pass_end, E_BADPASSLENGTH))
		return face.error(e);
	...
	if (pass_constraint_len) {
		...
		if (e.test(!m_cPConstraint, E_OUTOFMEM) || e.test(m_cPConstraint.status() != Code::loaded, int(m_cPConstraint.status()) + E_CODEFAILURE))
			return face.error(e);
		face.error_context(face.error_context() - 1);
	}
	if (m_numRules) {
		if (!readRanges(ranges, numRanges, e))
			return face.error(e);
		if (!readRules(rule_map, numEntries, precontext, sort_keys,
					o_constraint, rcCode, o_actions, aCode, face, pt, e))
			return false;
	}
...
bool Pass::readRules(const byte *rule_map, const size_t num_entries,
	...
	if (e.test(!(m_rules && m_codes && m_progs), E_OUTOFMEM))
		return face.error(e);
	...
	for (size_t n = m_numRules; r >= m_rules; --n, --r, ac_end = ac_begin, rc_end = rc_begin) {
		...
		if (ac_begin > ac_end || ac_begin > ac_data_end || ac_end > ac_data_end || rc_begin > rc_end || rc_begin > rc_data_end || rc_end > rc_data_end || vm::Machine::Code::estimateCodeDataOut(ac_end - ac_begin + rc_end - rc_begin, 2, r->sort) > size_t(prog_pool_end - prog_pool_free))
			return false;
		...
		if (e.test(!r->action || !r->constraint, E_OUTOFMEM) || e.test(r->action->status() != Code::loaded, int(r->action->status()) + E_CODEFAILURE) || e.test(r->constraint->status() != Code::loaded, int(r->constraint->status()) + E_CODEFAILURE) || e.test(!r->constraint->immutable(), E_MUTABLECCODE))
			return face.error(e);
	}
	...
	if (e.test(!re, E_OUTOFMEM))
		return face.error(e);
	for (size_t n = num_entries; n; --n, ++re) {
		const ptrdiff_t rn = be::read<uint16>(rule_map);
		if (e.test(rn >= m_numRules, E_BADRULENUM))
			return face.error(e);
		re->rule = m_rules + rn;
	}
...
static int cmpRuleEntry(const void *a, const void *b) { return (*(RuleEntry *)a < *(RuleEntry *)b ? -1 : (*(RuleEntry *)b < *(RuleEntry *)a ? 1 : 0)); }
...
bool Pass::readStates(const byte *starts, const byte *states, const byte *o_rule_map, GR_MAYBE_UNUSED Face &face, Error &e) {
	...
	if (e.test(!m_startStates || !m_states || !m_transitions, E_OUTOFMEM))
		return face.error(e);
	// load start states
	for (uint16 *s = m_startStates,
				*const s_end = s + m_maxPreCtxt - m_minPreCtxt + 1;
			s != s_end; ++s) {
	...
	for (uint16 *t = m_transitions,
				*const t_end = t + m_numTransition * m_numColumns;
			t != t_end; ++t) {
	...
	for (size_t n = m_numStates; n; --n, ++s) {
		...
		s->rules = begin;
		s->rules_end = (end - begin <= FiniteStateMachine::MAX_RULES) ? end : begin + FiniteStateMachine::MAX_RULES;
...
bool Pass::runFSM(FiniteStateMachine &fsm, Slot *slot) const {
	...
	do {
		...
		if (slot->gid() >= m_numGlyphs || m_cols[slot->gid()] == 0xffffU || --free_slots == 0 || state >= m_numTransition)
			return free_slots != 0;
...
inline Slot *input_slot(const SlotMap &slots, const int n) {
...
inline Slot *output_slot(const SlotMap &slots, const int n) {
			...
			if (fsm.rules.size() != 0) {
				*fsm.dbgout << json::item << json::object;
				...
				if (r != re) {
					...
					if (r->rule->action->deletes())
						fsm.slots.collectGarbage(slot);
...
inline bool Pass::testPassConstraint(Machine &m) const {
	if (!m_cPConstraint)
		return true;
...
bool Pass::testConstraint(const Rule &r, Machine &m) const {
	...
	if (unsigned(r.sort + curr_context - r.preContext) > m.slotMap().size() || curr_context - r.preContext < 0)
		return false;
...
bool Pass::collisionShift(Segment *seg, int dir, json *const dbgout) const {
	...
	while (start) {
		...
		for (Slot *s = start; s; s = s->next()) {
			...
			if (start && (c->flags() & (SlotCollision::COLL_FIX | SlotCollision::COLL_KERN)) == SlotCollision::COLL_FIX && !resolveCollisions(seg, s, start, shiftcoll, false, dir, moved, hasCollisions, dbgout))
				return false;
			...
			if (hasCollisions || moved) {
				...
				if (hasCollisions) {
					...
					for (Slot *s = lend; s != lstart; s = s->prev()) {
						SlotCollision *c = seg->collisionInfo(s);
						if (start && (c->flags() & (SlotCollision::COLL_FIX | SlotCollision::COLL_KERN | SlotCollision::COLL_ISCOL)) == (SlotCollision::COLL_FIX | SlotCollision::COLL_ISCOL)) // ONLY if this glyph is still colliding
						{
				...
				if (moved) {
					...
					for (Slot *s = start; s != end; s = s->next()) {
						SlotCollision *c = seg->collisionInfo(s);
						if (start && (c->flags() & (SlotCollision::COLL_FIX | SlotCollision::COLL_TEMPLOCK | SlotCollision::COLL_KERN)) == SlotCollision::COLL_FIX && !resolveCollisions(seg, s, start, shiftcoll, false, dir, moved, hasCollisions, dbgout))
							return false;
...
bool Pass::collisionKern(Segment *seg, int dir, json *const dbgout) const {
	...
	for (Slot *s = seg->first(); s; s = s->next()) {
		...
		if (start && (c->flags() & (SlotCollision::COLL_KERN | SlotCollision::COLL_FIX)) == (SlotCollision::COLL_KERN | SlotCollision::COLL_FIX))
