...
const size_t FEAT_HEADER = sizeof(uint32) + 2 * sizeof(uint16) + sizeof(uint32),
			 FEATURE_SIZE = sizeof(uint32) + 2 * sizeof(uint16) + sizeof(uint32) + 2 * sizeof(uint16),
...
bool FeatureMap::readFeats(const Face &face) {
	...
	if (version < 0x00010000 ||
			p + m_numFeats * FEATURE_SIZE > feat_end) { //defensive
		m_numFeats = 0;
		return false;
	}
	...
	for (int i = 0, ie = m_numFeats; i != ie; i++) {
		...
		if (settings_offset > size_t(feat_end - feat_start) || settings_offset + num_settings * FEATURE_SETTING_SIZE > size_t(feat_end - feat_start)) {
			free(defVals);
			return false;
		}
...
bool SillMap::readFace(const Face &face) {
	if (!m_FeatureMap.readFeats(face))
		return false;
	if (!readSill(face))
		return false;
	return true;
...
bool SillMap::readSill(const Face &face) {
	...
	if (sill.size() < 12)
		return false;
	if (be::read<uint32>(p) != 0x00010000UL)
		return false;
	m_numLanguages = be::read<uint16>(p);
	m_langFeats = new LangFeaturePair[m_numLanguages];
	if (!m_langFeats || !m_FeatureMap.m_numFeats) {
		m_numLanguages = 0;
		return true;
	} //defensive

	p += 6; // skip the fast search
	if (sill.size() < m_numLanguages * 8U + 12)
		return false;
	...
	for (int i = 0; i < m_numLanguages; i++) {
		...
		if (offset + 8U * numSettings > sill.size() && numSettings > 0)
			return false;
		...
		if (pRef)
			pRef->applyValToFeature(langid, *feats);
...
uint32 FeatureRef::getFeatureVal(const Features &feats) const {
	if (m_index < feats.size() && m_face && &m_face->theSill().theFeatureMap() == feats.m_pMap)
