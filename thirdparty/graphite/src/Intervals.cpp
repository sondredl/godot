...
inline Zones::Exclusion Zones::Exclusion::split_at(float p) {
	Exclusion r(*this);
	r.xm = x = p;
	return r;
}

inline void Zones::Exclusion::left_trim(float p) {
	x = p;
}

inline Zones::Exclusion &Zones::Exclusion::operator+=(Exclusion const &rhs) {
	c += rhs.c;
	sm += rhs.sm;
	smx += rhs.smx;
	open = false;
...
inline uint8 Zones::Exclusion::outcode(float val) const {
	float p = val;
...
inline bool separated(float a, float b) {
	return a != b;
	//int exp;
	...
	for (iterator i = _exclusions.begin(), ie = _exclusions.end(); i != ie && e.x < e.xm; ++i) {
		...
		switch (oca ^ ocb) // What kind of overlap?
		{
			...
			case 1: // e overlaps on the rhs of i
				...
				if (!separated(i->xm, e.x))
					break;
				if (separated(i->x, e.x)) {
			...
			case 2: // e overlaps on the lhs of i
				...
				if (!separated(e.xm, i->x))
					return;
				if (separated(e.xm, i->xm))
					i = _exclusions.insert(i, i->split_at(e.xm));
				*i += e;
				return;
	...
	for (iterator i = _exclusions.begin(), ie = _exclusions.end(); i != ie; ++i) {
		...
		switch (oca ^ ocb) // What kind of overlap?
		{
			case 0: // i completely covers e
				if (separated(i->x, x)) {
					i = _exclusions.insert(i, i->split_at(x));
					++i;
				}
				GR_FALLTHROUGH;
				// no break
...
float Zones::closest(float origin, float &cost) const {
	...
	for (const_iterator i = start, ie = _exclusions.end(); i != ie; ++i)
		if (i->track_cost(best_c, best_x, origin))
			break;
...
inline float Zones::Exclusion::cost(float p) const {
