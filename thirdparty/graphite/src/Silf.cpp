...
namespace {
static const uint32 ERROROFFSET = 0xFFFFFFFF;
}
...
bool Silf::readGraphite(const byte *const silf_start, size_t lSilf, Face &face, uint32 version) {
	...
	if (e.test(maxGlyph >= face.glyphs().numGlyphs(), E_BADMAXGLYPH) || e.test(p + m_numJusts * 8 >= silf_end, E_BADNUMJUSTS)) {
		releaseBuffers();
		return face.error(e);
	}
	...
	if (e.test(p + sizeof(uint16) + sizeof(uint8) * 8 >= silf_end, E_BADENDJUSTS)) {
		releaseBuffers();
		return face.error(e);
	}
	...
	if (e.test(p >= silf_end, E_BADCRITFEATURES)) {
		releaseBuffers();
		return face.error(e);
	}
	be::skip<uint32>(p, be::read<uint8>(p)); // don't use scriptTag array.
	if (e.test(p + sizeof(uint16) + sizeof(uint32) >= silf_end, E_BADSCRIPTTAGS)) {
		releaseBuffers();
		return face.error(e);
	}
	...
	if (e.test(unsigned(p - silf_start) + m_numPseudo * (sizeof(uint32) + sizeof(uint16)) >= passes_start, E_BADNUMPSEUDO) || e.test(!m_pseudos, E_OUTOFMEM)) {
		releaseBuffers();
		return face.error(e);
	}
	...
	if (e || e.test(clen > unsigned(passes_start + silf_start - p), E_BADPASSESSTART) || e.test(!m_passes, E_OUTOFMEM)) {
		releaseBuffers();
		return face.error(e);
	}
	...
	for (size_t i = 0; i < m_numPasses; ++i) {
		...
		if (e.test(pass_start > pass_end, E_BADPASSSTART) || e.test(pass_start < passes_start, E_BADPASSSTART) || e.test(pass_end > lSilf, E_BADPASSEND)) {
			releaseBuffers();
			return face.error(e);
		}
		...
		if (i >= m_jPass)
			pt = PASS_TYPE_JUSTIFICATION;
		else if (i >= m_pPass)
			pt = PASS_TYPE_POSITIONING;
		else if (i >= m_sPass)
			pt = PASS_TYPE_SUBSTITUTE;
		else
			pt = PASS_TYPE_LINEBREAK;
...
template <typename T>
inline uint32 Silf::readClassOffsets(const byte *&p, size_t data_len, Error &e) {
	...
	if (e.test(be::peek<T>(p) != cls_off, E_MISALIGNEDCLASSES) || e.test(max_off > (data_len - cls_off) / sizeof(uint16), E_HIGHCLASSOFFSET))
		return ERROROFFSET;

	// Read in all the offsets.
	m_classOffsets = gralloc<uint32>(m_nClass + 1);
	if (e.test(!m_classOffsets, E_OUTOFMEM))
		return ERROROFFSET;
...
size_t Silf::readClassMap(const byte *p, size_t data_len, uint32 version, Error &e) {
	if (e.test(data_len < sizeof(uint16) * 2, E_BADCLASSSIZE))
		return ERROROFFSET;
	...
	if (e.test(m_nLinear > m_nClass, E_TOOMANYLINEAR) || e.test((m_nClass + 1) * (version >= 0x00040000 ? sizeof(uint32) : sizeof(uint16)) > (data_len - 4), E_CLASSESTOOBIG))
		return ERROROFFSET;
	...
	if (e.test(!m_classData, E_OUTOFMEM))
		return ERROROFFSET;
...
uint16 Silf::findPseudo(uint32 uid) const {
	for (int i = 0; i < m_numPseudo; i++)
		if (m_pseudos[i].uid == uid)
			return m_pseudos[i].gid;
	return 0;
}
...
uint16 Silf::getClassGlyph(uint16 cid, unsigned int index) const {
	...
	} else // input class being used for output. Shouldn't happen
	{
		for (unsigned int i = loc + 4; i < m_classOffsets[cid + 1]; i += 2)
			if (m_classData[i + 1] == index)
				return m_classData[i];
	}
	return 0;
}
...
bool Silf::runGraphite(Segment *seg, uint8 firstPass, uint8 lastPass, int dobidi) const {
	...
	for (size_t i = firstPass; i < lastPass; ++i) {
		...
		if ((i >= 32 || (seg->passBits() & (1 << i)) == 0 || m_passes[i].collisionLoops()) && !m_passes[i].runGraphite(m, fsm, reverse))
			return false;
		// only subsitution passes can change segment length, cached subsegments are short for their text
		if (m.status() != vm::Machine::finished || (seg->slotCount() && seg->slotCount() > maxSize))
			return false;
	}
	return true;
}
