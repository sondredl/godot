...
inline u32 read_literal(u8 const *&s, u8 const *const e, u32 l) {
	if (l == 15 && s != e) {
		u8 b = 0;
		do {
			l += b = *s++;
...
int lz4::decompress(void const *in, size_t in_size, void *out, size_t out_size) {
	...
	while (read_sequence(src, src_end, literal, literal_len, match_len,
		...
		if (pcpy < static_cast<u8 *>(out) || match_len > unsigned(out_size - LASTLITERALS)
				// Wrap around checks:
				|| out_size < LASTLITERALS || pcpy >= dst)
			return -1;
		if (dst > pcpy + sizeof(unsigned long) && align(match_len) <= out_size)
