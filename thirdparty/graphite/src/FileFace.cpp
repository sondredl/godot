...
FileFace::FileFace(const char *filename) :
	...
	if (!TtfUtil::GetHeaderInfo(tbl_offset, tbl_len))
		return;
	if (fseek(_file, long(tbl_offset), SEEK_SET))
		return;
	_header_tbl = (TtfUtil::Sfnt::OffsetSubTable *)gralloc<char>(tbl_len);
	if (_header_tbl) {
		if (fread(_header_tbl, 1, tbl_len, _file) != tbl_len)
			return;
		if (!TtfUtil::CheckHeader(_header_tbl))
			return;
	}
...
const void *FileFace::get_table_fn(const void *appFaceHandle, unsigned int name, size_t *len) {
	...
	if (tbl_offset > file_face._file_len || tbl_len > file_face._file_len - tbl_offset || fseek(file_face._file, long(tbl_offset), SEEK_SET) != 0)
		return 0;
