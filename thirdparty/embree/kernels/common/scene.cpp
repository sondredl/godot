...
void Scene::build_cpu_accels() {
	...
	if (flags_modified || new_enabled_geometry_types != enabled_geometry_types) {
		...
		if (getNumPrimitives(Geometry::MTY_INSTANCE_ARRAY, false))
			createInstanceArrayAccel();
		if (getNumPrimitives(Geometry::MTY_INSTANCE_ARRAY, true))
			createInstanceArrayMBAccel();
...
void Scene::commit_task() {
	...
	this->world = parallel_reduce(
			size_t(0), geometries.size(), GeometryCounts(),
...
void Scene::commit(bool join) {
	...
	try {
		...
		device->execute(join, [&]() {
			taskGroup->group.run([&] {
				tbb::parallel_for(
						size_t(0), size_t(1), size_t(1), [&](size_t) { commit_task(); }, ctx);
			});
