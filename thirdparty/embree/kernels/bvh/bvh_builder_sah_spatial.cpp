...
struct CreateLeafSpatial {
	...
	__forceinline CreateLeafSpatial(BVH *bvh) :
			bvh(bvh) {}
...
struct BVHNBuilderFastSpatialSAH : public Builder {
	...
	BVHNBuilderFastSpatialSAH(BVH *bvh, Scene *scene, const size_t sahBlockSize, const float intCost, const size_t minLeafSize, const size_t maxLeafSize, const size_t mode) :
			bvh(bvh), scene(scene), mesh(nullptr), prims0(scene->device, 0), settings(sahBlockSize, minLeafSize, min(maxLeafSize, Primitive::max_size() * BVH::maxLeafBlocks), travCost, intCost, DEFAULT_SINGLE_THREAD_THRESHOLD), splitFactor(scene->device->max_spatial_split_replications) {}
	...
	BVHNBuilderFastSpatialSAH(BVH *bvh, Mesh *mesh, const unsigned int geomID, const size_t sahBlockSize, const float intCost, const size_t minLeafSize, const size_t maxLeafSize, const size_t mode) :
			bvh(bvh), scene(nullptr), mesh(mesh), prims0(bvh->device, 0), settings(sahBlockSize, minLeafSize, min(maxLeafSize, Primitive::max_size() * BVH::maxLeafBlocks), travCost, intCost, DEFAULT_SINGLE_THREAD_THRESHOLD), splitFactor(scene->device->max_spatial_split_replications), geomID_(geomID) {}
	...
	void build() {
		...
		if (likely(usePreSplits)) {
			...
			pinfo = mesh ? createPrimRefArray_presplit<Mesh, Splitter>(mesh, maxGeomID, numOriginalPrimitives, prims0, bvh->scene->progressInterface) : createPrimRefArray_presplit<Mesh, Splitter>(scene, Mesh::geom_type, false, numOriginalPrimitives, prims0, bvh->scene->progressInterface);
			...
			pinfo = mesh ? createPrimRefArray(mesh, geomID_, numSplitPrimitives, prims0, bvh->scene->progressInterface) : createPrimRefArray(scene, Mesh::geom_type, false, numSplitPrimitives, prims0, bvh->scene->progressInterface);
