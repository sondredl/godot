...
struct CreateMSMBlurLeaf {
	...
	__forceinline CreateMSMBlurLeaf(BVH *bvh) :
			bvh(bvh) {}
...
struct CreateMSMBlurLeafGrid {
	...
	__forceinline CreateMSMBlurLeafGrid(Scene *scene, BVH *bvh, const SubGridBuildData *const sgrids) :
...
struct BVHNBuilderMBlurSAHGrid : public Builder {
	...
	PrimInfoMB createPrimRefArrayMSMBlurGrid(Scene *scene, mvector<PrimRefMB> &prims, BuildProgressMonitor &progressMonitor, BBox1f t0t1 = BBox1f(0.0f, 1.0f)) {
		...
		pinfoMB = parallel_for_for_prefix_sum1(
				pstate, iter, PrimInfoMB(empty), [&](GridMesh *mesh, const range<size_t> &r, size_t k, size_t geomID, const PrimInfoMB &base) -> PrimInfoMB {
