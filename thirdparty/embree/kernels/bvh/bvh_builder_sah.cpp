...
struct CreateLeaf {
	...
	__forceinline CreateLeaf(BVH *bvh) :
			bvh(bvh) {}
...
struct CreateLeafQuantized {
	...
	__forceinline CreateLeafQuantized(BVH *bvh) :
			bvh(bvh) {}
...
struct BVHNBuilderSAH : public Builder {
	...
	BVHNBuilderSAH(BVH *bvh, Scene *scene, const size_t sahBlockSize, const float intCost, const size_t minLeafSize, const size_t maxLeafSize,
			...
			bvh(bvh), scene(scene), mesh(nullptr), prims(scene->device, 0), settings(sahBlockSize, minLeafSize, min(maxLeafSize, Primitive::max_size() * BVH::maxLeafBlocks), travCost, intCost, DEFAULT_SINGLE_THREAD_THRESHOLD), gtype_(gtype), primrefarrayalloc(primrefarrayalloc) {}
	...
	void build() {
		...
		profile(2, PROFILE_RUNS, numPrimitives, [&](ProfileTimer &timer) {
			...
			PrimInfo pinfo = mesh ? createPrimRefArray(mesh, geomID_, numPrimitives, prims, bvh->scene->progressInterface) : createPrimRefArray(scene, gtype_, false, numPrimitives, prims, bvh->scene->progressInterface);
...
struct CreateLeafGrid {
	...
	__forceinline CreateLeafGrid(BVH *bvh, const SubGridBuildData *const sgrids) :
			bvh(bvh), sgrids(sgrids) {}
