	...
	*out_len = 0;
	...
	for (; *group_list != 0; group_list++) {
		...
		if (propose_group) {
			...
			p += 2;
			MBEDTLS_SSL_DEBUG_MSG(3, ("NamedGroup: %s ( %x )", mbedtls_ssl_named_group_to_str(*group_list), *group_list));
		}
	}
	...
	*out_len = 0;
	...
	for (size_t i = 0; ciphersuite_list[i] != 0; i++) {
		...
		*tls12_uses_ec |= mbedtls_ssl_ciphersuite_uses_ec(ciphersuite_info);
		...
		MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, add ciphersuite: %04x, %s", (unsigned int)cipher_suite, ciphersuite_info->name));
