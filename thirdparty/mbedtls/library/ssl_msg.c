...
int mbedtls_ssl_check_dtls_clihlo_cookie(
	...
	if (in[0] != MBEDTLS_SSL_MSG_HANDSHAKE || epoch != 0 ||
		...
		MBEDTLS_SSL_DEBUG_MSG(4, ("    type=%u epoch=%u fragment_offset=%u", in[0], (unsigned)epoch, (unsigned)fragment_offset));
		return MBEDTLS_ERR_SSL_DECODE_ERROR;
	}
	...
	if (59 + 1 + sid_len + 1 + cookie_len > in_len) {
		MBEDTLS_SSL_DEBUG_MSG(4, ("check cookie: cookie_len=%u > %u", (unsigned)cookie_len, (unsigned)(in_len - sid_len - 61)));
		return MBEDTLS_ERR_SSL_DECODE_ERROR;
	}
...
static int ssl_parse_record_header(mbedtls_ssl_context const *ssl,
	...
	if (tls_version > ssl->conf->max_tls_version) {
		MBEDTLS_SSL_DEBUG_MSG(1, ("TLS version mismatch: got %u, expected max %u", (unsigned)tls_version, (unsigned)ssl->conf->max_tls_version));
