...
static void ssl_tls13_select_ciphersuite(
	...
	*selected_ciphersuite_info = NULL;
	...
	MBEDTLS_SSL_DEBUG_MSG(1, ("No matched ciphersuite, psk_ciphersuite_id=%x, psk_hash_alg=%lx", (unsigned)psk_ciphersuite_id, (unsigned long)psk_hash_alg));
}
...
static int ssl_tls13_offered_psks_check_identity_match_ticket(
	...
	if (now < session->ticket_creation_time) {
		MBEDTLS_SSL_DEBUG_MSG(
				3, ("Invalid ticket creation time ( now = %" MBEDTLS_PRINTF_MS_TIME ", creation_time = %" MBEDTLS_PRINTF_MS_TIME " )", now, session->ticket_creation_time));
		goto exit;
	}
	...
	if (server_age > MBEDTLS_SSL_TLS1_3_MAX_ALLOWED_TICKET_LIFETIME * 1000) {
		MBEDTLS_SSL_DEBUG_MSG(
				3, ("Ticket age exceeds limitation ticket_age = %" MBEDTLS_PRINTF_MS_TIME, server_age));
		goto exit;
	}
	...
	if (age_diff < -MBEDTLS_SSL_TLS1_3_TICKET_AGE_TOLERANCE ||
		...
		MBEDTLS_SSL_DEBUG_MSG(
				3, ("Ticket age outside tolerance window ( diff = %" MBEDTLS_PRINTF_MS_TIME ")", age_diff));
		goto exit;
	}
...
static int ssl_tls13_parse_client_hello(mbedtls_ssl_context *ssl,
	...
	MBEDTLS_SSL_DEBUG_MSG(2, ("selected ciphersuite: %04x - %s", ((unsigned)handshake->ciphersuite_info->id), handshake->ciphersuite_info->name));
	...
	if (handshake->key_exchange_mode &
		...
		MBEDTLS_SSL_DEBUG_MSG(2, ("Select PSK ciphersuite: %04x - %s", ((unsigned)psk.ciphersuite_info->id), psk.ciphersuite_info->name));
...
static int ssl_tls13_write_key_share_ext(mbedtls_ssl_context *ssl,
	...
	*out_len = 0;
	...
	MBEDTLS_SSL_DEBUG_MSG(2, ("server hello, write selected_group: %s (%04x)", mbedtls_ssl_named_group_to_str(group), group));
...
static int ssl_tls13_write_new_session_ticket_body(mbedtls_ssl_context *ssl,
	...
	*out_len = 0;
	...
	if (ticket_lifetime > MBEDTLS_SSL_TLS1_3_MAX_ALLOWED_TICKET_LIFETIME) {
		MBEDTLS_SSL_DEBUG_MSG(
				1, ("Ticket lifetime (%u) is greater than 7 days.", (unsigned int)ticket_lifetime));
...
int mbedtls_ssl_tls13_handshake_server_step(mbedtls_ssl_context *ssl) {
	...
	MBEDTLS_SSL_DEBUG_MSG(2, ("tls13 server state: %s(%d)", mbedtls_ssl_states_str((mbedtls_ssl_states)ssl->state), ssl->state));
