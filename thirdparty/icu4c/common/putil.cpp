...
static UBool isValidOlsonID(const char *id) {
	...
	return id[idx] == 0 || uprv_strcmp(id, "PST8PDT") == 0 || uprv_strcmp(id, "MST7MDT") == 0 || uprv_strcmp(id, "CST6CDT") == 0 || uprv_strcmp(id, "EST5EDT") == 0;
}
...
enum { U_DAYLIGHT_NONE = 0,
	U_DAYLIGHT_JUNE = 1,
	U_DAYLIGHT_DECEMBER = 2 };
...
static const char *remapShortTimeZone(const char *stdID, const char *dstID, int32_t daylightType, int32_t offset) {
	...
	for (idx = 0; idx < UPRV_LENGTHOF(OFFSET_ZONE_MAPPINGS); idx++) {
		if (offset == OFFSET_ZONE_MAPPINGS[idx].offsetSeconds && daylightType == OFFSET_ZONE_MAPPINGS[idx].daylightType && strcmp(OFFSET_ZONE_MAPPINGS[idx].stdID, stdID) == 0 && strcmp(OFFSET_ZONE_MAPPINGS[idx].dstID, dstID) == 0) {
...
static char *searchForTZFile(const char *path, DefaultTZInfo *tzInfo) {
	...
	while ((dirEntry = readdir(dirp)) != nullptr) {
		...
		if (uprv_strcmp(dirName, SKIP1) != 0 && uprv_strcmp(dirName, SKIP2) != 0 && uprv_strcmp(TZFILE_SKIP, dirName) != 0 && uprv_strcmp(TZFILE_SKIP2, dirName) != 0) {
...
static const char *uprv_getPOSIXIDForCategory(int category) {
	...
	if (category == LC_MESSAGES || category == LC_CTYPE) {
		...
		if ((posixID == nullptr) || (uprv_strcmp("C", posixID) == 0) || (uprv_strcmp("POSIX", posixID) == 0)) {
...
int_getDefaultCodepage() {
	...
	EPT_CALL(QUSRJOBI)
	(&jobinfo, sizeof(jobinfo), "JOBI0400",
			"*                         ", "                ", &error);
...
uprv_dlsym_func(void *lib, const char *sym, UErrorCode *status) {
	...
	if (U_FAILURE(*status) || lib == nullptr)
		return nullptr;
