...
void _getLanguage(const char *localeID,
		ByteSink *sink,
		const char **pEnd,
		UErrorCode &status) {
...
void _getScript(const char *localeID,
		ByteSink *sink,
		const char **pEnd) {
	U_ASSERT(pEnd != nullptr);
	*pEnd = localeID;
...
void _getRegion(const char *localeID,
		ByteSink *sink,
		const char **pEnd) {
	U_ASSERT(pEnd != nullptr);
	*pEnd = localeID;
 ...
 */
void _getVariant(const char *localeID,
		char prev,
		ByteSink *sink,
		const char **pEnd,
		bool needSeparator,
...
ulocimp_getSubtags(
	...
	if (language != nullptr) {
		languageSink.emplace(language);
	}
	if (script != nullptr) {
		scriptSink.emplace(script);
	}
	if (region != nullptr) {
		regionSink.emplace(region);
	}
	if (variant != nullptr) {
		variantSink.emplace(variant);
	}
...
ulocimp_getSubtags(
	...
	if (region == nullptr && variant == nullptr && pEnd == nullptr) {
		return;
	}
	...
	if (variant == nullptr && pEnd == nullptr) {
		return;
	}
 ...
 */
void _canonicalize(const char *localeID,
		ByteSink &sink,
		uint32_t options,
		UErrorCode &err) {
	if (U_FAILURE(err)) {
...
bool isWellFormedLegacyKey(std::string_view key) {
	return std::all_of(key.begin(), key.end(), UPRV_ISALPHANUM);
}

bool isWellFormedLegacyType(std::string_view legacyType) {
