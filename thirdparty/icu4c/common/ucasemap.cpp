...
void toLower(int32_t caseLocale, uint32_t options,
	...
	if (caseLocale == UCASE_LOC_ROOT ||
			(caseLocale >= 0 ? !(caseLocale == UCASE_LOC_TURKISH || caseLocale == UCASE_LOC_LITHUANIAN) : (options & _FOLD_CASE_OPTIONS_MASK) == U_FOLD_CASE_DEFAULT)) {
...
ucasemap_internalUTF8ToLower(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_UNUSED const uint8_t *src, int32_t srcLength,
...
static void U_CALLCONV
ucasemap_internalUTF8ToUpper(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_UNUSED const uint8_t *src, int32_t srcLength,
		icu::ByteSink &sink, icu::Edits *edits,
		UErrorCode &errorCode) {
...
static void U_CALLCONV
ucasemap_internalUTF8Fold(int32_t /* caseLocale */, uint32_t options, UCASEMAP_BREAK_ITERATOR_UNUSED const uint8_t *src, int32_t srcLength,
		icu::ByteSink &sink, icu::Edits *edits,
		UErrorCode &errorCode) {
	toLower(
			-1, options,
...
void ucasemap_mapUTF8(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_PARAM const char *src, int32_t srcLength,
	...
	stringCaseMapper(caseLocale, options, UCASEMAP_BREAK_ITERATOR reinterpret_cast<const uint8_t *>(src), srcLength, sink, edits, errorCode);
	sink.Flush();
...
int32_t
ucasemap_mapUTF8(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_PARAM char *dest, int32_t destCapacity,
	...
	int32_t reslen = ByteSinkUtil::viaByteSinkToTerminatedChars(
			...
			[&](ByteSink &sink, UErrorCode &status) {
				stringCaseMapper(caseLocale, options, UCASEMAP_BREAK_ITERATOR reinterpret_cast<const uint8_t *>(src), srcLength, sink, edits, status);
			},
			errorCode);
...
ucasemap_utf8ToLower(const UCaseMap *csm,
	...
	return ucasemap_mapUTF8(
			csm->caseLocale, csm->options, UCASEMAP_BREAK_ITERATOR_NULL dest, destCapacity,
			src, srcLength,
...
ucasemap_utf8FoldCase(const UCaseMap *csm,
		...
		UErrorCode *pErrorCode) {
	return ucasemap_mapUTF8(
			UCASE_LOC_ROOT, csm->options, UCASEMAP_BREAK_ITERATOR_NULL dest, destCapacity,
			src, srcLength,
...
void CaseMap::utf8ToLower(
		...
		UErrorCode &errorCode) {
	ucasemap_mapUTF8(
			ustrcase_getCaseLocale(locale), options, UCASEMAP_BREAK_ITERATOR_NULL src.data(), src.length(),
...
void CaseMap::utf8Fold(
		...
		UErrorCode &errorCode) {
	ucasemap_mapUTF8(
			UCASE_LOC_ROOT, options, UCASEMAP_BREAK_ITERATOR_NULL src.data(), src.length(),
...
int32_t CaseMap::utf8ToLower(
		...
		UErrorCode &errorCode) {
	return ucasemap_mapUTF8(
			ustrcase_getCaseLocale(locale), options, UCASEMAP_BREAK_ITERATOR_NULL dest, destCapacity,
			src, srcLength,
...
int32_t CaseMap::utf8Fold(
		...
		UErrorCode &errorCode) {
	return ucasemap_mapUTF8(
			UCASE_LOC_ROOT, options, UCASEMAP_BREAK_ITERATOR_NULL dest, destCapacity,
			src, srcLength,
