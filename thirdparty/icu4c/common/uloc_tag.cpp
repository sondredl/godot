...
bool _isAlphaString(const char *s, int32_t len) {
	int32_t i;
	for (i = 0; i < len; i++) {
		if (!ISALPHA(*(s + i))) {
			return false;
...
bool _isNumericString(const char *s, int32_t len) {
	int32_t i;
	for (i = 0; i < len; i++) {
		if (!ISNUMERIC(*(s + i))) {
			return false;
...
bool _isAlphaNumericString(const char *s, int32_t len) {
	int32_t i;
	for (i = 0; i < len; i++) {
...
bool _isAlphaNumericStringLimitedLength(const char *s, int32_t len, int32_t min, int32_t max) {
	if (len < 0) {
...
bool ultag_isLanguageSubtag(const char *s, int32_t len) {
	/*
...
bool _isExtlangSubtag(const char *s, int32_t len) {
	/*
...
bool ultag_isScriptSubtag(const char *s, int32_t len) {
	/*
...
bool ultag_isRegionSubtag(const char *s, int32_t len) {
	/*
...
bool _isVariantSubtag(const char *s, int32_t len) {
	/*
...
bool _isSepListOf(bool (*test)(const char *, int32_t), const char *s, int32_t len) {
	const char *p = s;
...
bool ultag_isVariantSubtags(const char *s, int32_t len) {
...
bool _isPrivateuseVariantSubtag(const char *s, int32_t len) {
	/*
...
bool _isExtensionSingleton(const char *s, int32_t len) {
	/*
...
bool _isExtensionSubtag(const char *s, int32_t len) {
	/*
	 * extension     = singleton 1*("-" (2*8alphanum))
	 */
...
bool ultag_isExtensionSubtags(const char *s, int32_t len) {
	return _isSepListOf(&_isExtensionSubtag, s, len);
}

namespace {

bool _isPrivateuseValueSubtag(const char *s, int32_t len) {
	/*
...
bool ultag_isPrivateuseValueSubtags(const char *s, int32_t len) {
	return _isSepListOf(&_isPrivateuseValueSubtag, s, len);
}

bool ultag_isUnicodeLocaleAttribute(const char *s, int32_t len) {
	/*
...
bool ultag_isUnicodeLocaleAttributes(const char *s, int32_t len) {
	return _isSepListOf(&ultag_isUnicodeLocaleAttribute, s, len);
}

bool ultag_isUnicodeLocaleKey(const char *s, int32_t len) {
	/*
	 * key = alphanum alpha ;
	 */
	if (len < 0) {
...
bool _isUnicodeLocaleTypeSubtag(const char *s, int32_t len) {
	/*
	 * alphanum{3,8}
	 */
	return _isAlphaNumericStringLimitedLength(s, len, 3, 8);
...
bool ultag_isUnicodeLocaleType(const char *s, int32_t len) {
	/*
...
bool _isTKey(const char *s, int32_t len) {
	/*
	 * tkey = alpha digit ;
	 */
	if (len < 0) {
...
bool _isTValue(const char *s, int32_t len) {
	/*
...
bool _isTransformedExtensionSubtag(int32_t &state, const char *s, int32_t len) {
...
bool _isUnicodeExtensionSubtag(int32_t &state, const char *s, int32_t len) {
...
bool _isStatefulSepListOf(bool (*test)(int32_t &, const char *, int32_t), const char *s, int32_t len) {
	int32_t state = 0;
	const char *p;
	const char *start = s;
	int32_t subtagLen = 0;
...
bool ultag_isTransformedExtensionSubtags(const char *s, int32_t len) {
	return _isStatefulSepListOf(&_isTransformedExtensionSubtag, s, len);
}

bool ultag_isUnicodeExtensionSubtags(const char *s, int32_t len) {
...
bool _addVariantToList(VariantListEntry **first, icu::LocalPointer<VariantListEntry> var) {
	if (*first == nullptr) {
		var->next = nullptr;
		*first = var.orphan();
	} else {
...
bool _addAttributeToList(AttributeListEntry **first, AttributeListEntry *attr) {
	bool bAdded = true;
...
bool _addExtensionToList(ExtensionListEntry **first, ExtensionListEntry *ext, bool localeToBCP) {
	bool bAdded = true;
...
void _initializeULanguageTag(ULanguageTag *langtag) {
	int32_t i;
...
void _appendLanguageToLanguageTag(const char *localeID, icu::ByteSink &sink, bool strict, UErrorCode &status) {
	...
	if (buf.isEmpty()) {
		...
		for (int32_t i = 0; i < UPRV_LENGTHOF(DEPRECATEDLANGS); i += 2) {
			...
			if (buf.length() < static_cast<int32_t>(uprv_strlen(DEPRECATEDLANGS[i])))
				break;
...
void _appendScriptToLanguageTag(const char *localeID, icu::ByteSink &sink, bool strict, UErrorCode &status) {
	UErrorCode tmpStatus = U_ZERO_ERROR;
...
void _appendRegionToLanguageTag(const char *localeID, icu::ByteSink &sink, bool strict, UErrorCode &status) {
	UErrorCode tmpStatus = U_ZERO_ERROR;
...
void _appendVariantsToLanguageTag(const char *localeID, icu::ByteSink &sink, bool strict, bool &hadPosix, UErrorCode &status) {
	if (U_FAILURE(status)) {
		return;
	}
...
void _appendKeywordsToLanguageTag(const char *localeID, icu::ByteSink &sink, bool strict, bool hadPosix, UErrorCode &status) {
	if (U_FAILURE(status)) {
		return;
	}
 ...
 */
void _appendLDMLExtensionAsKeywords(const char *ldmlext, ExtensionListEntry **appendTo, icu::MemoryPool<ExtensionListEntry> &extPool, icu::MemoryPool<icu::CharString> &kwdBuf, bool &posixVariant, UErrorCode &status) {
	if (U_FAILURE(status)) {
		return;
	}
...
void _appendKeywords(ULanguageTag *langtag, icu::ByteSink &sink, UErrorCode &status) {
	if (U_FAILURE(status)) {
		return;
	}
...
void _appendPrivateuseToLanguageTag(const char *localeID, icu::ByteSink &sink, bool strict, bool /*hadPosix*/, UErrorCode &status) {
	if (U_FAILURE(status)) {
		return;
	}
