...
static UBool changesWhenCasefolded(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	...
	if (c >= 0) {
		...
		return U_SUCCESS(errorCode) &&
				0 != u_strCompare(nfd.getBuffer(), nfd.length(), dest, destLength, false);
	}
}
#endif
...
static UBool isCanonSegmentStarter(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	UErrorCode errorCode = U_ZERO_ERROR;
	const Normalizer2Impl *impl = Normalizer2Factory::getNFCImpl(errorCode);
	return U_SUCCESS(errorCode) && impl->ensureCanonIterData(errorCode) &&
...
static UBool isIDCompatMathStart(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	if (c < ID_COMPAT_MATH_START[0]) {
		return false;
	} // fastpath for common scripts
...
static UBool isIDCompatMathContinue(const BinaryProperty &prop, UChar32 c, UProperty /*which*/) {
	for (int32_t i = 0; i < UPRV_LENGTHOF(ID_COMPAT_MATH_CONTINUE); i += 2) {
		if (c < ID_COMPAT_MATH_CONTINUE[i]) {
			return false;
		} // below range start
		if (c < ID_COMPAT_MATH_CONTINUE[i + 1]) {
			return true;
		} // below range limit
	}
...
static UBool isModifierCombiningMark(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	for (int32_t i = 0; i < UPRV_LENGTHOF(MODIFIER_COMBINING_MARK); i += 2) {
		if (c < MODIFIER_COMBINING_MARK[i]) {
			return false;
		} // below range start
		if (c < MODIFIER_COMBINING_MARK[i + 1]) {
			return true;
		} // below range limit
	}
	return false;
...
u_getIDTypes(UChar32 c, UIdentifierType *types, int32_t capacity, UErrorCode *pErrorCode) {
	...
	if ((value & UPROPS_ID_TYPE_FORBIDDEN) == UPROPS_ID_TYPE_FORBIDDEN ||
		...
		if (capacity > 0) {
			UIdentifierType t;
			switch (value) {
				case UPROPS_ID_TYPE_NOT_CHARACTER:
					t = U_ID_TYPE_NOT_CHARACTER;
					break;
				case UPROPS_ID_TYPE_DEPRECATED:
					t = U_ID_TYPE_DEPRECATED;
					break;
				case UPROPS_ID_TYPE_DEFAULT_IGNORABLE:
					t = U_ID_TYPE_DEFAULT_IGNORABLE;
					break;
				case UPROPS_ID_TYPE_NOT_NFKC:
					t = U_ID_TYPE_NOT_NFKC;
					break;
				case UPROPS_ID_TYPE_INCLUSION:
					t = U_ID_TYPE_INCLUSION;
					break;
				case UPROPS_ID_TYPE_RECOMMENDED:
					t = U_ID_TYPE_RECOMMENDED;
					break;
				default:
