...
UBool DictionaryBreakEngine::handles(UChar32 c, const char *) const {
...
public:
	PossibleWord() :
			count(0), prefix(0), offset(-1), mark(0), current(0) {}
	~PossibleWord() {}
...
ThaiBreakEngine::divideUpDictionaryRange(UText *text,
	...
	while (U_SUCCESS(status) && (current = static_cast<int32_t>(utext_getNativeIndex(text))) < rangeEnd) {
		...
		if (static_cast<int32_t>(utext_getNativeIndex(text)) < rangeEnd && cpWordLength < THAI_ROOT_COMBINE_THRESHOLD) {
			...
			if (words[wordsFound % THAI_LOOKAHEAD].candidates(text, fDictionary, rangeEnd) <= 0 && (cuWordLength == 0 || words[wordsFound % THAI_LOOKAHEAD].longestPrefix() < THAI_PREFIX_COMBINE_THRESHOLD)) {
...
LaoBreakEngine::divideUpDictionaryRange(UText *text,
	...
	while (U_SUCCESS(status) && (current = static_cast<int32_t>(utext_getNativeIndex(text))) < rangeEnd) {
		...
		if (static_cast<int32_t>(utext_getNativeIndex(text)) < rangeEnd && cpWordLength < LAO_ROOT_COMBINE_THRESHOLD) {
			...
			// no preceding word, or the non-word shares less than the minimum threshold
			// of characters with a dictionary word, then scan to resynchronize
			if (words[wordsFound % LAO_LOOKAHEAD].candidates(text, fDictionary, rangeEnd) <= 0 && (cuWordLength == 0 || words[wordsFound % LAO_LOOKAHEAD].longestPrefix() < LAO_PREFIX_COMBINE_THRESHOLD)) {
				// Look for a plausible word boundary
				int32_t remaining = rangeEnd - (current + cuWordLength);
				UChar32 pc;
				UChar32 uc;
...
BurmeseBreakEngine::divideUpDictionaryRange(UText *text,
	...
	while (U_SUCCESS(status) && (current = static_cast<int32_t>(utext_getNativeIndex(text))) < rangeEnd) {
		...
		if (static_cast<int32_t>(utext_getNativeIndex(text)) < rangeEnd && cpWordLength < BURMESE_ROOT_COMBINE_THRESHOLD) {
			...
			// no preceding word, or the non-word shares less than the minimum threshold
			// of characters with a dictionary word, then scan to resynchronize
			if (words[wordsFound % BURMESE_LOOKAHEAD].candidates(text, fDictionary, rangeEnd) <= 0 && (cuWordLength == 0 || words[wordsFound % BURMESE_LOOKAHEAD].longestPrefix() < BURMESE_PREFIX_COMBINE_THRESHOLD)) {
				// Look for a plausible word boundary
				int32_t remaining = rangeEnd - (current + cuWordLength);
				UChar32 pc;
				UChar32 uc;
...
KhmerBreakEngine::divideUpDictionaryRange(UText *text,
	...
	while (U_SUCCESS(status) && (current = static_cast<int32_t>(utext_getNativeIndex(text))) < rangeEnd) {
		...
		if (static_cast<int32_t>(utext_getNativeIndex(text)) < rangeEnd && cpWordLength < KHMER_ROOT_COMBINE_THRESHOLD) {
			...
			// no preceding word, or the non-word shares less than the minimum threshold
			// of characters with a dictionary word, then scan to resynchronize
			if (words[wordsFound % KHMER_LOOKAHEAD].candidates(text, fDictionary, rangeEnd) <= 0 && (cuWordLength == 0 || words[wordsFound % KHMER_LOOKAHEAD].longestPrefix() < KHMER_PREFIX_COMBINE_THRESHOLD)) {
				// Look for a plausible word boundary
				int32_t remaining = rangeEnd - (current + cuWordLength);
				UChar32 pc;
				UChar32 uc;
	...
	if (static_cast<uint32_t>(bestSnlp.elementAti(numCodePts)) == kuint32max) {
		...
		if (U_SUCCESS(status)) {
			...
			for (int32_t i = prev.elementAti(numCodePts); i > 0; i = prev.elementAti(i)) {
				...
				if (!fSkipSet.containsKey(inString.tempSubString(codeUnitIdx, length)) && (!isKatakana(inString.char32At(inString.moveIndex32(codeUnitIdx, -1))) || !isKatakana(inString.char32At(codeUnitIdx)))) {
