...
void AliasDataBuilder::readAlias(
 ...
 */
void AliasDataBuilder::readLanguageAlias(
		UResourceBundle *alias,
		UniqueCharStrings *strings,
		LocalMemory<const char *> &types,
		LocalMemory<int32_t> &replacementIndexes,
 ...
 * replacement script.
 */
void AliasDataBuilder::readScriptAlias(
		UResourceBundle *alias,
		UniqueCharStrings *strings,
		LocalMemory<const char *> &types,
		LocalMemory<int32_t> &replacementIndexes,
 ...
 * replacement regions.
 */
void AliasDataBuilder::readTerritoryAlias(
		UResourceBundle *alias,
		UniqueCharStrings *strings,
		LocalMemory<const char *> &types,
		LocalMemory<int32_t> &replacementIndexes,
 ...
 * replacement variant.
 */
void AliasDataBuilder::readVariantAlias(
		UResourceBundle *alias,
		UniqueCharStrings *strings,
		LocalMemory<const char *> &types,
		LocalMemory<int32_t> &replacementIndexes,
 ...
 * replacement regions.
 */
void AliasDataBuilder::readSubdivisionAlias(
		UResourceBundle *alias,
		UniqueCharStrings *strings,
		LocalMemory<const char *> &types,
		LocalMemory<int32_t> &replacementIndexes,
...
public:
	AliasReplacer(UErrorCode &status) :
			language(nullptr), script(nullptr), region(nullptr), extensions(nullptr),
			// store value in variants only once
			variants(nullptr, ([](UElement e1, UElement e2) -> UBool { return 0 == uprv_strcmp((const char *)e1.pointer, (const char *)e2.pointer); }), status),
			data(nullptr) {
	}
	~AliasReplacer() {
	}
...
void AliasReplacer::parseLanguageReplacement(
...
bool AliasReplacer::replaceLanguage(
	...
	for (int32_t variant_index = 0;
		...
		parseLanguageReplacement(replacement,
				...
				status);
		replacedLanguage =
				(replacedLanguage != nullptr && uprv_strcmp(replacedLanguage, "und") == 0) ? language : replacedLanguage;
		...
		if (item.isNull() && U_SUCCESS(status)) {
			item.adoptInsteadAndCheckErrorCode(
					new CharString(replacement,
							static_cast<int32_t>(firstSpace - replacement), status),
					status);
		}
		if (U_FAILURE(status)) {
			return false;
...
bool AliasReplacer::replaceSubdivision(
...
bool AliasReplacer::replaceTransformedExtensions(
		CharString &transformedExtensions, CharString &output, UErrorCode &status) {
...
Locale &Locale::init(const char *localeID, UBool canonicalize) {
	...
	do {
		...
		err = U_ZERO_ERROR;
		length = canonicalize ? uloc_canonicalize(localeID, fullName, sizeof(fullNameBuffer), &err) : uloc_getName(localeID, fullName, sizeof(fullNameBuffer), &err);
...
void Locale::minimizeSubtags(bool favorScript, UErrorCode &status) {
	if (U_FAILURE(status)) {
		return;
	}
...
void Locale::setUnicodeKeywordValue(StringPiece keywordName,
		StringPiece keywordValue,
		UErrorCode &status) {
	if (U_FAILURE(status)) {
		return;
