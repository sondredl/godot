...
enumAlgNames(AlgorithmicRange *range,
	...
	switch (range->type) {
		...
		case 1: {
			...
			length = static_cast<uint16_t>(prefixLength + writeFactorSuffix(factors, count, s, static_cast<uint32_t>(start) - range->start, indexes, elementBases, elements, suffix, static_cast<uint16_t>(sizeof(buffer) - prefixLength)));
