...
#define UCNV_SISO_STATE(cnv)                                                                                                                          \
	((cnv)->sharedData->mbcs.outputType == MBCS_OUTPUT_2_SISO ? (int8_t)(cnv)->mode : (cnv)->sharedData->mbcs.outputType == MBCS_OUTPUT_DBCS_ONLY ? 1 \
																																				  : -1)
...
extFromUUseMapping(UBool useFallback, uint32_t value, UChar32 firstCP) {
	return ((value & UCNV_EXT_FROM_U_STATUS_MASK) != 0 ||
