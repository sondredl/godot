...
//========================================
UBool UnicodeString::doEquals(const char16_t *text, int32_t len) const {
...
UBool UnicodeString::doEqualsSubstring(int32_t start,
		int32_t length,
...
UnicodeString::doAppend(const char16_t *srcChars, int32_t srcStart, int32_t srcLength) {
	...
	if (srcLength <= getCapacity() - oldLength && isBufferWritable()) {
		...
		if (srcLength <= 4) {
			...
			if (srcLength > 1)
				arr[oldLength + 1] = srcChars[1];
			if (srcLength > 2)
				arr[oldLength + 2] = srcChars[2];
			if (srcLength > 3)
				arr[oldLength + 3] = srcChars[3];
			setLength(newLength);
			return *this;
		}
	} else {
 ...
 * Replaceable API
 */
void UnicodeString::handleReplaceBetween(int32_t start,
		int32_t limit,
...
// Miscellaneous
//========================================
UBool UnicodeString::cloneArrayIfNeeded(int32_t newCapacity,
...
UBool UnicodeStringAppendable::appendString(const char16_t *s, int32_t length) {
