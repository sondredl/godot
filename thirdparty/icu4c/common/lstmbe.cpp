...
public:
	Array2D() :
			memory_(nullptr), data_(nullptr), d1_(0), d2_(0) {}
...
LSTMData::LSTMData(UResourceBundle *rb, UErrorCode &status) :
		fDict(nullptr), fType(UNKNOWN), fName(nullptr), fBundle(rb) {
	if (U_FAILURE(status)) {
		return;
	}
	if (IEEE_754 != 1) {
