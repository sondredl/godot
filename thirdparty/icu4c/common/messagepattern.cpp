...
template <typename T, int32_t stackCapacity>
void MessagePatternList<T, stackCapacity>::copyFrom(
...
template <typename T, int32_t stackCapacity>
UBool MessagePatternList<T, stackCapacity>::ensureCapacityForOneMore(int32_t oldLength, UErrorCode &errorCode) {
...
MessagePattern::MessagePattern(const MessagePattern &other) :
		UObject(other), aposMode(other.aposMode), msg(other.msg), partsList(nullptr), parts(nullptr), partsLength(0), numericValuesList(nullptr), numericValues(nullptr), numericValuesLength(0), hasArgNames(other.hasArgNames), hasArgNumbers(other.hasArgNumbers), needsAutoQuoting(other.needsAutoQuoting) {
...
bool MessagePattern::operator==(const MessagePattern &other) const {
	...
	return aposMode == other.aposMode &&
			msg == other.msg &&
...
void MessagePattern::parseDouble(int32_t start, int32_t limit, UBool allowInfinity,
		UParseError *parseError, UErrorCode &errorCode) {
	if (U_FAILURE(errorCode)) {
		return;
	}
...
UBool MessagePattern::isChoice(int32_t index) {
	char16_t c;
	return ((c = msg.charAt(index++)) == u_c || c == u_C) &&
...
UBool MessagePattern::isPlural(int32_t index) {
	char16_t c;
	return ((c = msg.charAt(index++)) == u_p || c == u_P) &&
...
UBool MessagePattern::isSelect(int32_t index) {
	char16_t c;
	return ((c = msg.charAt(index++)) == u_s || c == u_S) &&
			((c = msg.charAt(index++)) == u_e || c == u_E) &&
			((c = msg.charAt(index++)) == u_l || c == u_L) &&
			((c = msg.charAt(index++)) == u_e || c == u_E) &&
			((c = msg.charAt(index++)) == u_c || c == u_C) &&
...
UBool MessagePattern::isOrdinal(int32_t index) {
	char16_t c;
	return ((c = msg.charAt(index++)) == u_o || c == u_O) &&
			((c = msg.charAt(index++)) == u_r || c == u_R) &&
