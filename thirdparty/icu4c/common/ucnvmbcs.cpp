...
ucnv_MBCSGetUnicodeSetForUnicode(const UConverterSharedData *sharedData,
	...
	ucnv_MBCSGetFilteredUnicodeSetForUnicode(
			sharedData, sa, which,
			sharedData->mbcs.outputType == MBCS_OUTPUT_DBCS_ONLY ? UCNV_SET_FILTER_DBCS_ONLY : UCNV_SET_FILTER_NONE,
			pErrorCode);
}
	...
	if ((cnv->options & _MBCS_OPTION_GB18030) != 0) {
		...
		for (i = 0; i < UPRV_LENGTHOF(gb18030Ranges); range += 4, ++i) {
			if (range[0] <= static_cast<uint32_t>(cp) && static_cast<uint32_t>(cp) <= range[1]) {
				...
				bytes[3] = static_cast<char>(0x30 + linear % 10);
				linear /= 10;
				bytes[2] = static_cast<char>(0x81 + linear % 126);
				linear /= 126;
				bytes[1] = static_cast<char>(0x30 + linear % 10);
				linear /= 10;
	...
	if (mbcsTable->outputType == MBCS_OUTPUT_EXT_ONLY) {
		...
		if (noFromU) {
			uint32_t stage1Length =
					mbcsTable->unicodeMask & UCNV_HAS_SUPPLEMENTARY ? 0x440 : 0x40;
...
ucnv_MBCSWriteSub(UConverterFromUnicodeArgs *pArgs,
	...
	if (cnv->subChar1 != 0 &&
			(cnv->sharedData->mbcs.extIndexes != nullptr ? cnv->useSubChar1 : (cnv->invalidUCharBuffer[0] <= 0xff))) {
