...
static void U_CALLCONV initAliasData(UErrorCode &errCode) {
	...
	if (gMainTable.optionTableSize > 0 && reinterpret_cast<const UConverterAliasOptions *>(table + currOffset)->stringNormalizationType < UCNV_IO_NORM_TYPE_COUNT) {
		/* Faster table */
	...
	for (;;) {
		...
		if (result < 0) {
			...
			if (containsOption) {
				...
				*containsOption = static_cast<UBool>((containsCnvOptionInfo && ((gMainTable.untaggedConvArray[mid] & UCNV_CONTAINS_OPTION_BIT) != 0)) || !containsCnvOptionInfo);
			}
