...
LSR::LSR(char prefix, const char *lang, const char *scr, const char *r, int32_t f,
		...
		language(nullptr), script(nullptr), region(r), regionIndex(indexForRegion(region)), flags(f) {
...
LSR::LSR(StringPiece lang, StringPiece scr, StringPiece r, int32_t f,
		UErrorCode &errorCode) :
		language(nullptr), script(nullptr), region(nullptr), regionIndex(indexForRegion(r.data())), flags(f) {
	if (U_SUCCESS(errorCode)) {
		CharString data;
...
LSR::LSR(LSR &&other) noexcept :
		language(other.language), script(other.script), region(other.region), owned(other.owned), regionIndex(other.regionIndex), flags(other.flags), hashCode(other.hashCode) {
	if (owned != nullptr) {
