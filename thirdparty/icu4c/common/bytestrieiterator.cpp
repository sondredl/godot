...
BytesTrie::Iterator::Iterator(const BytesTrie &trie, int32_t maxStringLength,
		UErrorCode &errorCode) :
		bytes_(trie.bytes_), pos_(trie.pos_), initialPos_(trie.pos_), remainingMatchLength_(trie.remainingMatchLength_), initialRemainingMatchLength_(trie.remainingMatchLength_), str_(nullptr), maxLength_(maxStringLength), value_(0), stack_(nullptr) {
	if (U_FAILURE(errorCode)) {
		return;
	}
	str_ = new CharString();
...
UBool BytesTrie::Iterator::hasNext() const { return pos_ != nullptr || !stack_->isEmpty(); }
