...
bool _isKeywordValue(const char *key, const char *value, int32_t value_len) {
	if (key[1] == '\0') {
		// one char key
...
void _copyExtensions(const Locale &from, icu::StringEnumeration *keywords,
...
void _clearUAttributesAndKeyType(Locale &locale, UErrorCode &errorCode) {
	if (U_FAILURE(errorCode)) {
		return;
	}
...
void _setUnicodeExtensions(Locale &locale, const CharString &value, UErrorCode &errorCode) {
	if (U_FAILURE(errorCode)) {
		return;
	}
