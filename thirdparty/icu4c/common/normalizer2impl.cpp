...
ReorderingBuffer::ReorderingBuffer(const Normalizer2Impl &ni, UnicodeString &dest,
		...
		impl(ni), str(dest), start(str.getBuffer(8)), reorderStart(start), limit(start), remainingCapacity(str.getCapacity()), lastCC(0) {
...
UBool ReorderingBuffer::equals(const char16_t *otherStart, const char16_t *otherLimit) const {
	...
	return length == static_cast<int32_t>(otherLimit - otherStart) &&
...
UBool Normalizer2Impl::compose(const char16_t *src, const char16_t *limit,
...
UBool Normalizer2Impl::composeUTF8(uint32_t options, UBool onlyContiguous,
		const uint8_t *src, const uint8_t *limit,
		ByteSink *sink, Edits *edits, UErrorCode &errorCode) const {
	U_ASSERT(limit != nullptr);
	UnicodeString s16;
...
CanonIterData::CanonIterData(UErrorCode &errorCode) :
		mutableTrie(umutablecptrie_open(0, 0, &errorCode)), trie(nullptr), canonStartSets(uprv_deleteUObject, nullptr, errorCode) {}
