...
UCharsTrie::Iterator::Iterator(const UCharsTrie &trie, int32_t maxStringLength,
		UErrorCode &errorCode) :
		uchars_(trie.uchars_), pos_(trie.pos_), initialPos_(trie.pos_), remainingMatchLength_(trie.remainingMatchLength_), initialRemainingMatchLength_(trie.remainingMatchLength_), skipValue_(false), maxLength_(maxStringLength), value_(0), stack_(nullptr) {
	if (U_FAILURE(errorCode)) {
		return;
	}
	stack_ = new UVector32(errorCode);
...
UBool UCharsTrie::Iterator::hasNext() const { return pos_ != nullptr || !stack_->isEmpty(); }
