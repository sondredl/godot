...
png_crc_finish_critical(png_structrp png_ptr, png_uint_32 skip,
	...
	if (png_crc_error(png_ptr, handle_as_ancillary) != 0) {
		...
		if (handle_as_ancillary || PNG_CHUNK_ANCILLARY(png_ptr->chunk_name) != 0 ? (png_ptr->flags & PNG_FLAG_CRC_ANCILLARY_NOWARN) == 0 : (png_ptr->flags & PNG_FLAG_CRC_CRITICAL_USE) != 0) {
...
png_handle_PLTE(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
	 ...
	 */
		...
		const unsigned max_palette_length =
				(png_ptr->color_type == PNG_COLOR_TYPE_PALETTE) ? 1U << png_ptr->bit_depth : PNG_MAX_PALETTE_LENGTH;

		/* The cast is safe because 'length' is less than
		 * 3*PNG_MAX_PALETTE_LENGTH
		 */
		const unsigned num = (length > 3U * max_palette_length) ? max_palette_length : (unsigned)length / 3U;
